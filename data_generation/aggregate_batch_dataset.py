...
def angular_spectrum(angles_3_src,switch=None):
    ...
    if "azimuth" in switch:
        ...
        azi_angular_spectrum[1,:]=np.max((np.exp(-(angle_distance_1**2/sigma**2)),np.exp(-(angle_distance_2**2/sigma**2))),axis=0)
        azi_angular_spectrum[2,:]=np.max((np.exp(-(angle_distance_1**2/sigma**2)),np.exp(-(angle_distance_2**2/sigma**2)),np.exp(-(angle_distance_3**2/sigma**2))),axis=0)
        ...
        ele_angular_spectrum[1,:]=np.max((np.exp(-(angle_distance_1**2/sigma_ele**2)),np.exp(-(angle_distance_2**2/sigma_ele**2))),axis=0)
        ele_angular_spectrum[2,:]=np.max((np.exp(-(angle_distance_1**2/sigma_ele**2)),np.exp(-(angle_distance_2**2/sigma_ele**2)),np.exp(-(angle_distance_3**2/sigma_ele**2))),axis=0)
    ...
    if "noisy" in file_name:
        ...
        for i in range(room_start,room_start+parallel_jobs): #File name used to iterate through rooms
            ...
            if room_mixture.shape[1] == 2:
                ...
                snr_src_1_mix.append(abc["room_nos"]["room_"+str(i)]["nsmix_snr_src_1"][()])
                snr_src_1_arr_3_mic.append(abc["room_nos"]["room_"+str(i)]["SINR_S1"][()])
                snr_src_2_arr_3_mic.append(abc["room_nos"]["room_"+str(i)]["SINR_S2"][()])
                snr_src_3_arr_3_mic.append(abc["room_nos"]["room_"+str(i)]["SINR_S3"][()])
...
new_file_anno=h5py.File(path_noisy_mix+"/noisy_mixtures/"+"D6_0111_aggregated_mixture_annotations.hdf5","w")
...
for file_name in os.listdir(path_noisy_mix):
    if "_anno_rooms_" in file_name:
        ...
        for room_no in abc["rirs_save_anno"].keys(): #Keys used to iterate through rooms.
            ...
            azimuth_spectrum_3_sources=angular_spectrum(azimuth_3_sources,switch="azimuth")
            ...
            room_id.create_dataset("azimuth_spectrum",(3,360),data=azimuth_spectrum_3_sources)
...
plt.clf()
fig,ax2=plt.subplots()
ax2.hist(np.array(snr_src_1_arr_3_mic),bins=np.arange(-50,100,step=5),color="orange")
ax2.set_ylabel("No of samples")
...
ax3.hist(np.array(snr_src_2_arr_3_mic),bins=np.arange(-50,100,step=5),color="orange")
ax3.set_xlabel("SINR dB SRC 2")
plt.savefig(path_save_figs+"SINR_S2.jpeg")

plt.clf()
fig,ax4=plt.subplots()
ax4.hist(np.array(snr_src_3_arr_3_mic),bins=np.arange(-50,100,step=5),color="orange")
ax4.set_xlabel("SINR dB SRC 3")
plt.savefig(path_save_figs+"SINR_S3.jpeg")

plt.clf()
#Average histogram
concat_histograms=np.concatenate((np.array(snr_src_1_arr_3_mic),np.array(snr_src_2_arr_3_mic),np.array(snr_src_3_arr_3_mic)),axis=0)
