...
def calculate_source_doa_1d(rec_pos,rec_bc,src_pos,rec_bc_mic,no_of_rec_room,no_of_source_room): #One mic array and multiple source positions in the room.
    azimuth=[]
...
class util_receiver():
    ...
    def mic_defination_array(self,rotation_matrix,barycenter):
        ...
        return panel_1_mic_1+barycenter, panel_1_mic_2+barycenter, ((panel_1_mic_1+barycenter)+(panel_1_mic_2+barycenter))/2


    def generate_receivers_rooms(self,room_dimension,different_no_receivers,saftey_distance,room_id,mic_in_ula=None):
        ...
        for x in range(different_no_receivers):
            ...
            barycenter=np.array([round(random.uniform(0.2,(room_dimension[0]-saftey_distance)),3),
            round(random.uniform(0.2,(room_dimension[1]-saftey_distance)),3),
            round(random.uniform(0.2,(room_dimension[2]-saftey_distance)),3)])
            ...
            barycenter_ypr=[random.randint(0,360),0,elevation_rotation_bc] #random.randint(0,360) Can also be selected from the unit vector selection.
            ...
            mic_pos_1, mic_pos_2, mic_bc = self.mic_defination_array(rotation_mat,barycenter)
...
class util_source():
    ...
    def generate_source_room(self,room_dimension,different_no_sources,saftey_distance,barycenter):
        ...
        for i in range(different_no_sources):
            tmp_cord = np.array([round(random.uniform(0.2, x_safteydistance), 3),
            ...
            while (tmp_distance_1 < 0.6): #and tmp_distance_2 < 0.8 and tmp_distance_3 < 0.8) :
                tmp_cord = np.array([round(random.uniform(0.2, x_safteydistance), 3),
                                     round(random.uniform(0.2, y_safteydistance), 3),
                                     round(random.uniform(0.2, z_safteydistance), 3)])
                tmp_distance_1=distance.euclidean(tmp_cord,barycenter[0,:])
...
class conf_files(util_room,util_receiver):
    ...
    def room_file(self):
        ...
        saftey_distance=0.2 #Saftey distance between source and microphone with the room walls.
        ...
        for x in tqdm(range(self.number_rooms)):
            ...
            rec_pos,li_bc,li_bc_mic,rec_pos_ypr=self.receiver_file.generate_receivers_rooms(return_dimension,no_of_reciver_per_room,saftey_distance,'room_' + str(x)) #random.choice([2,3]) for x in range(no_of_reciver_per_room)
            ...
            li_nsc=self.source_file.fake_source_rooom(return_dimension,1,saftey_distance,li_bc)

            dict_file_receiver['room_' + str(x)] = {'barycenter':li_bc.tolist(),'rec_pos':rec_pos.tolist(),'barycenter_mic':li_bc_mic.tolist(),'rec_pos_ypr':rec_pos_ypr.tolist()}#,'directivity':'AKG_c414K'}
            ...
            #Calculate GT DoA 
            azimuth = calculate_source_doa_1d(rec_pos,li_bc,li_sc,li_bc_mic,no_of_rec_room=1,no_of_source_room=3)

            dict_file_source['room_' + str(x)]={'source_pos':li_sc.tolist(),'source_ypr':li_sc_ypr.tolist(),'theta_1d':azimuth.tolist()}#,'directivity':random.sample(['Genelec_8020','Yamaha_DXR8','HATS_4128C'],k=3)}
        ...
        with open('conf_room_setup_W.yml', 'w') as file:
            documents = yaml.dump(dict_file, file)
        with open('conf_receivers_W.yml','w') as file_1:
            documents = yaml.dump(dict_file_receiver,file_1)
        with open('conf_source_W.yml','w') as file_2:
            documents = yaml.dump(dict_file_source,file_2)
        with open('conf_noise_source_W.yml','w') as file_3:
