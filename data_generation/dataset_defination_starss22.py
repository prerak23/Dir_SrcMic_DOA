...
class util_receiver():
    ...
    def generate_receivers_rooms(self,room_dimension,different_no_receivers,saftey_distance,room_id,mic_in_ula=None):
        ...
        for x in range(different_no_receivers):
            ...
            barycenter_ypr=[0,0,0] #random.randint(0,360) Can also be selected from the unit vector selection.
...
class conf_files(util_room,util_receiver):
    ...
    def room_file(self):
        ...
        for x in tqdm(range(self.number_rooms)):
            ...
            dict_file_receiver['room_' + str(x)] = {'barycenter':li_bc.tolist(),'rec_pos':rec_pos.tolist(),'barycenter_mic':li_bc_mic.tolist()}#'rec_pos_ypr':rec_pos_ypr.tolist(),'directivity':['EM_32_5','EM_32_9']}
            ...
            dict_file_source['room_' + str(x)]={'source_pos':li_sc.tolist(),'source_ypr':li_sc_ypr.tolist(),'theta_1d':azimuth.tolist(),'directivity':random.sample(['HATS_4128C','Genelec_8020','Yamaha_DXR8'],3)}
