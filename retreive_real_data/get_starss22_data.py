    ...
    for files in os.listdir(path + folders):
        ...
        if no_repeated_frames.shape[0] > 20:
            ...
            if len(k) > 0:
                for j in k:
                    if (
                        abc[np.where(abc[:, 0] == j[0])[0], 1] == 0
                        or abc[np.where(abc[:, 0] == j[0])[0], 1] == 1
                    ) and (
                        abc[np.where(abc[:, 0] == j[1])[0], 1] == 0
                        or abc[np.where(abc[:, 0] == j[1])[0], 1] == 1
                    ):
                        file_wav = files.split(".")[0]
                        s, data = wavfile.read(
                            path_audio + folders + "/" + file_wav + ".wav"
                        )
                        data_audio.append(
                            data[int(j[0]) * 2400 : (int(j[1]) + 1) * 2400, :]
                        )
                        data_cord.append(
                            polar2cart(
                                int(abc[np.where(abc[:, 0] == j[0])[0], 3]),
                                int(abc[np.where(abc[:, 0] == j[0])[0], 4]),
                            )
                        )
