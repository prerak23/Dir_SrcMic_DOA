...
house_1 = {
    ...
    "room_2": {},
    "room_3": {},
}  # [{'quite':[],'noiseCond1':[],'noiseCond2':[],'noiseCond3':[]}, {'quite':[],'noiseCond1':[],'noiseCond2':[],'noiseCond3':[]} , {'quite':[],'noiseCond1':[],'noiseCond2':[],'noiseCond3':[]}]
...
for h_no in np.arange(nHouse):  # Iterate through homes
    ...
    for r_no in np.arange(nRoom):  # Iterate through rooms, each home has 3 rooms.
        ...
        for spk_no in np.arange(
            nSpk
        ):  # Iterate through number of speakers, each home has 3 speakers
            print("speaker", spk_no)
            hc[h_no]["room_" + str(r_no + 1)][enc[h_no][spk_no]] = {}
            hc_ano[h_no]["room_" + str(r_no + 1)][enc[h_no][spk_no]] = {}

            for pos_no in np.arange(
                nPos
            ):  # Iterate through each speaker positions,each speaker has 5 position.
                ...
                #'bc_mic':bc_mic,
                hc_ano[h_no]["room_" + str(r_no + 1)][enc[h_no][spk_no]][
                    "position_" + str(pos_no + 1)
                ] = {
                    "doa_angle": doa_angle,
                    "array_cord_bc": array_cord_bc,
                    "spk_cord": spk_cord,
                    "spk_dir": spk_dir,
                    "mic_1": voicehome2_mic_1,
                    "mic_2": voicehome2_mic_2,
                }
