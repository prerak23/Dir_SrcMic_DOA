...
def srp_phat(err,acc,rec_pos,bc,azimuth_estm,X,j,l,spk_dist):
    ...
    doa = pra.doa.algorithms.get("SRP")(L, fs, nfft, num_src=number_of_sources, dim=2, mode="far",azimuth=np.deg2rad(np.linspace(0,180,num=360)))
            ...
            for pos_no in np.arange(nPos):
                ...
                for j in vh.item().get('room_'+str(r_enc)).get(spk_enc).get('position_'+str(pos_no+1)):
                    if "noiseCond1_" in j:
                        ...
                        stft_2_mic[0,:,:]= stft(data_2_mic[0,:],fs=16000,nperseg=2048)[2]
                        stft_2_mic[1,:,:]= stft(data_2_mic[1,:],fs=16000,nperseg=2048)[2]

                        acc,err=srp_phat(err,acc,mics,bc,doa_angle_gt,stft_2_mic,j,l,spk_dist)
