...
mic_11 = (
    np.array([1.5051453880902617, -3.9210377912882475, 2.571758278209442e-16]) / 100
)
...
for b in np.arange(data.shape[0]):
    ...
    delay = pra.experimental.localization.tdoa(
        data_16k[0, :], data_16k[1, :], distance_between_mic / 100, interp=4, fs=16000
    )

    # if delay > -0.0008:
    all_tdoa.append([azimuth_src, delay])

    estimated_angle = np.abs(
        180
        - np.rad2deg(np.arccos((delay * speed_of_sound) / (distance_between_mic / 100)))
    )  # For DCASE and voice home use this
