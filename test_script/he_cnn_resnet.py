...
def doa_metric(predicted_spectrum, azimuth_gt,switch="train"):
    ...
    for no in np.arange(batch_size):
        ...
        predicted_angle=np.abs(180-predicted_angle) #For DIRHA use this, for Dcase change this
        ...
        if np.abs(predicted_angle-azimuth_gt[no][0]) <= tolerance:
            ...
            mean_batch_err=np.abs(predicted_angle-azimuth_gt[no][0])+mean_batch_err
            mean_batch_loc=np.abs(predicted_angle-azimuth_gt[no][0])+mean_batch_loc
        else:
            mean_batch_err=np.abs(predicted_angle-azimuth_gt[no][0])+mean_batch_err

    if "val" in switch or "test" in switch:
        return ((batch_acc*100)/batch_size), (mean_batch_err/batch_size), predicted_angle_arr,(mean_batch_loc/batch_acc)
    else:
