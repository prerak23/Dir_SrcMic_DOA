...
dataset="D3_1011"
data=h5py.File(path+dataset+"/noisy_mixtures/"+dataset+"_aggregated_mixture.hdf5")
coord=h5py.File(path+dataset+"/noisy_mixtures/"+dataset+"_aggregated_mixture_annotations.hdf5")
...
rooms=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/data_generation/val_random_ar_only_source_1_included_new.npy")
...
for j in np.arange(stft_feat.shape[0]):
    ...
    data_unsq=torch.unsqueeze(stft_feat[j,:,:,:],axis=0) #Add one extra dimension at axis=0, so that it behaves like batch of just one example, when input to the network
    #print(data_unsq.shape,j)
    output_spectrum=net(data_unsq.cuda().float()) #Procees one example at a time, due to memory constraints of the GPU
