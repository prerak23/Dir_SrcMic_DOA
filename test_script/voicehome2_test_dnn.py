...
time_segment_stft=33 # For 1 second of signal and 50 % overlap with window_len of 2048
max_channel_data=2
device="cpu"
ACT_NONE = 0
...
voicehome_home1=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/test_scripts/real_data/voiceHome2_house_1.npy",allow_pickle=True)
voicehome_home1_ano=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/test_scripts/real_data/voiceHome2_house_1_ano.npy",allow_pickle=True)

voicehome_home2=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/test_scripts/real_data/voiceHome2_house_2.npy",allow_pickle=True)
voicehome_home2_ano=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/test_scripts/real_data/voiceHome2_house_2_ano.npy",allow_pickle=True)

voicehome_home3=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/test_scripts/real_data/voiceHome2_house_3.npy",allow_pickle=True)
voicehome_home3_ano=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/test_scripts/real_data/voiceHome2_house_3_ano.npy",allow_pickle=True)

voicehome_home4=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/test_scripts/real_data/voiceHome2_house_4.npy",allow_pickle=True)
voicehome_home4_ano=np.load("/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/test_scripts/real_data/voiceHome2_house_4_ano.npy",allow_pickle=True)



vh_arr=[voicehome_home1,voicehome_home2,voicehome_home3,voicehome_home4] #List of variables
vh_ano_arr=[voicehome_home1_ano,voicehome_home2_ano,voicehome_home3_ano,voicehome_home4_ano]
    ...
    for r_no in np.arange(nRoom):
        ...
        for spk_no in np.arange(nSpk):
            ...
            for pos_no in np.arange(nPos):
                ...
                #Retreive gt angle
                doa_angle_gt=[[vh_ano.item().get('room_'+str(r_enc)).get(spk_enc).get('position_'+str(pos_no+1)).get('doa_angle')]]

                #Retreive mic positions
                mics=np.array([vh_ano.item().get('room_'+str(r_enc)).get(spk_enc).get('position_'+str(pos_no+1)).get('mic_1'),vh_ano.item().get('room_'+str(r_enc)).get(spk_enc).get('position_'+str(pos_no+1)).get('mic_2')]).T

                #Retreive barycenter position
                bc=vh_ano.item().get('room_'+str(r_enc)).get(spk_enc).get('position_'+str(pos_no+1)).get('array_cord_bc')

                #Retreive speaker coordinate
                spk_dist=vh_ano.item().get('room_'+str(r_enc)).get(spk_enc).get('position_'+str(pos_no+1)).get('spk_cord')

                #Retreive specific .wav file
                for j in vh.item().get('room_'+str(r_enc)).get(spk_enc).get('position_'+str(pos_no+1)):

                    if "noiseCond1_" in j: #If noiseCond1_ is satisfied : means it is for quite noise.
                        ...
                        real_data_anno_stack=np.vstack((real_data_anno_stack,doa_angle_gt))
