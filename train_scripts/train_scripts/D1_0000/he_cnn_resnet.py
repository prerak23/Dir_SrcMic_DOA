...
def train(
    ...
    for batch_idx, sample_batched in enumerate(data_loader_train):
        ...
        data, azi_spectrum_gt, azi_gt = (
            sample_batched["bnsample"].float(),
            sample_batched["azimuth_spectrum"].float().to(device=device),
            sample_batched["azimuth"].float().to(device="cpu"),
        )
...
train_data = dl.binuaral_dataset(
    "/home/psrivastava/source_localization/doa_estimation/voicehome2_arr/data_generation/train_random_ar_only_source_1_included_new.npy"
)
val_data = dl.binuaral_dataset(
...
path = "/srv/storage/talc3@talc-data.nancy/multispeech/calcul/users/psrivast/ICASSP/dcase_arr/training/D1_0000/antoine_norm_less_data/"

for epoch in tqdm(range(no_of_epochs)):
    epoch_loss_arr, batch_loss_arr, epoch_err_arr, epoch_acc_arr = train(
        net,
        train_dl,
        optimizer,
        epoch,
    ...
    np.save(path + "val_loss" + "_.npy", epoch_loss_arr_val)
    np.save(path + "acc" + "_.npy", epoch_acc_val_arr)
    np.save(path + "err" + "_.npy", epoch_err_val_arr)
        ...
        torch.save(
            {
                ...
                "epoch": epoch,
                "loss": epoch_loss_arr_val[-1],
            },
            path + "he_cnn_save_best_sh_" + str(epoch) + ".pt",
        )
        save_best_val = epoch_loss_arr_val[-1]
        np.save(path + "predicted_angle_arr" + "_.npy", predicted_angle_arr)
        np.save(path + "ouput_spectrum_arr" + "_.npy", output_arr)
