...
abcd = h5py.File(
    ...
    + "/noisy_mixtures/"
    + dataset_name
    + "_aggregated_mixture.hdf5",
    "r",
)  # Open the noisy mixture file
anno_file = h5py.File(
    path_root
    + dataset_name
    + "/noisy_mixtures/"
...
class binuaral_dataset(Dataset):
    def __init__(self, randomized_arr):
        ...
        self.random_arr = np.load(
            randomized_arr
        )  # Randomize the room_number and view points , therefore [(Room_0,vp_1),(Room_2,vp_4),(Room_3,vp_5),(Room_2,vp_3)]
    ...
    def __getitem__(self, item_):
        ...
        noisy_mixture_all_channels_ = np.zeros(
            (max_no_of_channels_in_dataset, length_of_mixture)
        )

        # channel_in_current_data=abcd['room_nos'][item[0]]['nsmix_f'].shape[1]

        noisy_mixture_all_channels_[:, :] = abcd["room_nos"][item[0]]["nsmix_f"][
            no_of_sources - 1, :, :
        ]  # number of sources in the mixture x number of channels present x length of signal
        ...
        azimuth_spectrum = anno_file["room_nos"][item[0]]["azimuth_spectrum"][
            no_of_sources - 1, :
        ]  #  3, 360    3: refers to 3 sources in the mixture
        ...
        sample = {
            "bnsample": noisy_mixture_all_channels_,
            "room": int(item[0].split("_")[1]),
